...
flag_upsets = [u for u in upsets if full_zero_dim(u) == False]
...
with open("sl3rk16summary.txt",'a') as ff:
	for k in range(673,876): #876
		...
		H = list(grassmannian_hwvs_for_upset(data,upsets[k],verbose=True))
		ff.write(str(len(H))+'\n')
		print('len(H) is '+ str(len(H)))
'''
def Grassmannian_hwvs(k,mdata,verbose=True):
	for hwt in grassmannian_hwvs_for_upset(data,flag_upsets[k],verbose):
		yield hwt

def border_apolarity_110(T,reps,C,r,k):
	with open("sl3rk16res{}_flag.txt".format(k),'w') as ff:
