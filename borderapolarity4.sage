...
def border_apolarity_cycl_inv(T,reps,C,r):
    ...
    cand111 = []
    for xs in product(*list(map(enumerate,[cand110]*3))):
...
def grassmannian_hwvs_for_upset(mdata,up,verbose=False):
    ...
    for nzsi,nzs in enumerate(product(*[combinations(list(range(p[-1])),m) \
            for p,m in up])):
        ...
        for q,nz in zip(up,nzs):
            ...
            W[wt] = (M[p[0]]*t, m,f)
        cur = block_matrix([[B for B,m,f in list(W.values())]],subdivide=False)
...
def tensor_cycl(T):
    ...
    for i,j,k in product(list(range(m)),list(range(n)),list(range(s))):
        S[j][k,i] = T[i][j,k]
    return S
...
def minors_ideal(M,r):
    ...
    def rec(M,r):
        ...
        e = Counter(list(M.dict().values())).most_common(1)[0][0]
...
def minors_sparse(M,r):
    ...
    a,b = M.dimensions()
    out = []
    for ix in combinations(list(range(a)),r):
