...
# This file was *autogenerated* from the file borderapolarity.sage
from sage.all_cmdline import *   # import sage library

_sage_const_3 = Integer(3); _sage_const_2 = Integer(2); _sage_const_1 = Integer(1); _sage_const_0 = Integer(0)# In this file, modules of semisimple lie algebras represented in code in the
# following way:
...
def border_apolarity_cycl_inv(T,reps,C,r):
    ...
    adim = len(T)
    cand110 = []
    for ghwv in grassmannian_hwvs(mdata, em.dimensions()[_sage_const_0 ]-r):
        cand = em*ghwv
    ...
    cand111 = []
    for xs in product(*map(enumerate,[cand110]*_sage_const_3 )):
        ixs = tuple(i for i,x in xs)
        if ixs != min([ixs,ixs[_sage_const_1 :]+ixs[:_sage_const_1 ],ixs[_sage_const_2 :]+ixs[:_sage_const_2 ]]):
        ...
        if W.rank() <= W.dimensions()[_sage_const_0 ] - r:
            cand110.append(W)
...
def border_apolarity_110data(T,reps,C,missing=_sage_const_2 ):
    reps = reps[missing+_sage_const_1 :] + reps[:missing]
    print 'Afull..'
...
def grassmannian_hwvs_for_upset(mdata,up,verbose=False):
    ...
    A = mdata['a']
    M = mdata['M']
    ssrank = len(A[_sage_const_0 ])

    if verbose: 
        upd = dict(up)
        missing = [p[_sage_const_0 ] for p in mdata['tot'] if p[-_sage_const_1 ] > upd.get(p,_sage_const_0 )]
    ...
    for nzsi,nzs in enumerate(product(*[combinations(range(p[-_sage_const_1 ]),m) \
            for p,m in up])):
        ...
        for q,nz in zip(up,nzs):
            ...
            for j,ks in enumerate(zip(nz,nz[_sage_const_1 :]+(f,))):
                inc = (ks[_sage_const_1 ]-ks[_sage_const_0 ]-_sage_const_1 )*(j+_sage_const_1 )
                t[ks[_sage_const_0 ]+_sage_const_1 :ks[_sage_const_1 ],:j+_sage_const_1 ] = matrix(R,ks[_sage_const_1 ]-ks[_sage_const_0 ]-_sage_const_1 ,j+_sage_const_1 ,R.gens()[pi:pi+inc])
                pi += inc 
        ...
        for wt,v in W.items():
            ...
            for k in range(ssrank):
                ...
                if curd is not None:
                    Braise,mr,fr = curd
                    if mr == fr: continue
                    mm = Braise.augment(A[_sage_const_0 ][k]*B)
                    eqs.extend(minors_sparse(mm,mr+_sage_const_1 ))
                elif rwt in M:
                    eqs.extend((A[_sage_const_0 ][k]*B).coefficients())
...
def grassmannian_hwvs_upsets(mdata,sz):
    ...
    for p in tot:
        ...
        for k in range(_sage_const_2 ,len(wtg.incoming_edges(p))+_sage_const_1 ):
            for es in combinations(wtg.incoming_edges(p),k):
                ...
                lp.add_constraint(p[-_sage_const_1 ] - lp[p] <= lp.sum(
                    q[-_sage_const_1 ] - lp[q] for _,q,xr in es) + kdim)
    ...
    def dfs(i):
        if i == len(tot):
            yield [(p,int(lp.get_min(lp[p]))) for p in tot if int(lp.get_min(lp[p])) > _sage_const_0 ]
            return
        p = tot[i]
        wt, mult = p
    ...
    print len(list(dfs(_sage_const_0 )))
...
def AB_grass_restrict_ok(W,adim,r):
    bdim = W.dimensions()[_sage_const_0 ] // adim
    M = matrix_11_to_21(W,adim)
    eqs = matrix_rank_le_eqs(M, M.dimensions()[_sage_const_0 ] - r)
    ...
    eqs.extend(matrix_rank_le_eqs(M, M.dimensions()[_sage_const_0 ] - r))
    if _sage_const_1  in eqs or (M.base_ring() is not QQ and _sage_const_1  in M.base_ring().ideal(eqs)): 
        return None
...
def matrix_11_to_21(B,a):
    ...
    for I,s in B.nonzero_positions():
        ...
        for k in range(a):
            ...
            ix = (( binomial(ma+_sage_const_1 ,_sage_const_2 )+mi )*b+j,s*a+k)
            W[ix] = W.get(ix,_sage_const_0 ) + v
    W = matrix(B.base_ring(),binomial(a+_sage_const_1 ,_sage_const_2 )*b,S*a,W)
    return W

# A is bc x S, B is ca x U, C is ab x V
def matrix_to_111(A,B,C):
    a = int(sqrt(B.dimensions()[_sage_const_0 ]*C.dimensions()[_sage_const_0 ]/A.dimensions()[_sage_const_0 ]))
    b = C.dimensions()[_sage_const_0 ] // a
    c = B.dimensions()[_sage_const_0 ] // a
    W = {}
    for i in range(a):
        for I,l in A.nonzero_positions():
            j,k = I // c, I % c
            W[((i*b+j)*c+k, i*A.dimensions()[_sage_const_1 ]+l)] = A[j*c+k,l]
    for j in range(b):
        for I,l in B.nonzero_positions():
            k,i = I // a, I % a
            W[((i*b+j)*c+k, a*A.dimensions()[_sage_const_1 ] + j*B.dimensions()[_sage_const_1 ]+l)] = B[k*a+i,l]
    for k in range(c):
        for I,l in C.nonzero_positions():
            i,j = I // b, I % b
            W[((i*b+j)*c+k, a*A.dimensions()[_sage_const_1 ] + b*B.dimensions()[_sage_const_1 ] +\
                    k*C.dimensions()[_sage_const_1 ]+l)] = C[i*b+j,l]
    W = matrix(A.base_ring(),a*b*c,a*A.dimensions()[_sage_const_1 ]+b*B.dimensions()[_sage_const_1 ]+c*C.dimensions()[_sage_const_1 ],W)
    return W
...
def module_product(a,b):
    return [[x.tensor_product(identity_matrix(QQ,y.dimensions()[_sage_const_0 ],sparse=True)) +
            identity_matrix(QQ,x.dimensions()[_sage_const_0 ],sparse=True).tensor_product(y)
...
def weight_decomposition(a,C):
    tot = simultaneous_eigenspace(a[_sage_const_2 ])

    totkey = lambda wt: tuple(C.solve_right(vector(QQ,wt)).list())
    tot.sort(key=lambda p: totkey(p[_sage_const_1 ]))

    M = {wt:V for V,wt in tot}
    tot = [(wt,V.dimensions()[_sage_const_1 ]) for V,wt in tot]
    ...
    for p in tot:
        ...
        for i,x in enumerate(a[_sage_const_0 ]):
            ...
            if wt2 in M:
                wtg.add_edge(p,(wt2,M[wt2].dimensions()[_sage_const_1 ]),restrict_map(x,M[wt2],B))
...
def tensor_cycl(T):
    m = len(T)
    n,s = T[_sage_const_0 ].dimensions()
...
def matrix_rank_le_eqs(M,r):
    if M.base_ring() is QQ:
        ...
        return list(minors_ideal(M,r+_sage_const_1 ).basis)
...
def minors_ideal(M,r):
    ...
    if r <= lo:
        return Rorig.ideal(_sage_const_1 )
    r -= lo
    M = M[lo:,lo:] 
    if M.is_zero():
        return Rorig.ideal()
    ...
    def rec(M,r):
        ...
        e = Counter(M.dict().values()).most_common(_sage_const_1 )[_sage_const_0 ][_sage_const_0 ]

        R1 = MPolyQuoLoc(R.base(), R._ideal+e._p, R._den, nilpotents=False )
        t = M.change_ring(R1)
        t,lo = sparse_elimination_by_units(t)
        I1 = rec(t[lo:,lo:],r-lo) if lo < r else R.base().ideal(_sage_const_1 )

        R2 = MPolyQuoLoc(R.base(), R._ideal, R._den*e._p, nilpotents=False )
        t = M.change_ring(R2)
        t,lo = sparse_elimination_by_units(t)
        I2 = rec(t[lo:,lo:],r-lo) if lo < r else R.base().ideal(_sage_const_1 )

        return I1.intersection(I2)

    S = MPolyQuoLoc(R,I,_sage_const_1 )
    if R is Rorig:
...
def minors_sparse(M,r):
    M = M[[i for i in range(M.dimensions()[_sage_const_0 ]) if not M[i].is_zero()], [j for j
        in range(M.dimensions()[_sage_const_1 ]) if not M[:,j].is_zero()]]
    a,b = M.dimensions()
    out = []
...
def sparse_elimination_by_units(M):
    ...
    while True:
        ...
        for i in M.column(r)[r+_sage_const_1 :].nonzero_positions():
            i += r+_sage_const_1 
...
class MPolyQuoLoc(CommutativeRing):
    ...
    def _element_constructor_(self, x):
        if isinstance(x, MPolyQuoLocElement):
            return MPolyQuoLocElement(self,x._p,_sage_const_0 )*\
                MPolyQuoLocElement(self,x.parent()._den,_sage_const_0 ).inverse_of_unit()**x._k
        else:
            return MPolyQuoLocElement(self,self.base()(x),_sage_const_0 )
...
class MPolyQuoLocElement(CommutativeRingElement):
    def __init__(self, parent, p, k):
        ...
        while k > _sage_const_0 :
            try:
                p = parent.base()(singular.lift(J,p).sage()[-_sage_const_1 ,_sage_const_0 ])
    ...
    def inverse_of_unit(self):
        ...
        if _sage_const_1  not in S:
            raise ArithmeticError("element is not a unit")
        e = self.parent().base()(singular.lift(J,self.parent()._den**k).sage()[-_sage_const_1 ,_sage_const_0 ])
