...
def border_apolarity_gen(T,Lg,reps,r):
    dims = (len(T),) + T[_sage_const_0 ].dimensions()
    global cands
    cands = [[] for i in range(_sage_const_3 )]
    for missing in range(_sage_const_3 ):
        mdata,em = border_apolarity_110data(T,Lg,reps,missing)
        for ghwv in grassmannian_hwvs(mdata, em.dimensions()[_sage_const_0 ]-r):
            cand = em*ghwv
            cand = AB_grass_restrict_ok(cand,dims[(missing+_sage_const_1 )%_sage_const_3 ],r,fast=False)
            if cand is not None:
...
def border_apolarity_110data(T,Lg,reps,missing=_sage_const_2 ):
...
def grassmannian_hwvs_for_upset(mdata,up,verbose=False):
    ...
    M,S = mdata['M'],QQ
    ssdim = len(A[_sage_const_0 ])

    if verbose: 
        upd = dict(up)
        missing = [p[_sage_const_0 ]+(p[-_sage_const_1 ]-upd.get(p,_sage_const_0 ),) 
                for p in mdata['tot'] if p[-_sage_const_1 ] > upd.get(p,_sage_const_0 )]
    ...
    if all((p[-_sage_const_1 ]-m)*m == _sage_const_0  for p,m in up):
        ...
        for j,nzs in enumerate(product(*[combinations(range(p[-_sage_const_1 ]),m) \
                for p,m in up])):
            if verbose:
                print j,
                sys.stdout.flush()
            ...
            names = [x+str(i) for x,q,nz in zip(alphagen(),up,nzs)
                    for i in range(sum((ks[_sage_const_1 ]-ks[_sage_const_0 ]-_sage_const_1 )*(j+_sage_const_1 ) 
                        for j,ks in enumerate(zip(nz,nz[_sage_const_1 :]+(q[_sage_const_0 ][-_sage_const_1 ],))))) ]
            R = PolynomialRing(S,names,implementation='singular') if len(names) > _sage_const_0  else S
            ...
            for wt,v in W.items():
                B,m,f = v
                if m==_sage_const_0 : continue # for clarity and future changes, should never proc
                def raisewt(wt,k):
                ...
                for k in range(ssdim):
                    ...
                    if curd is not None:
                        ...
                        eqs.extend(minors_sparse(A[_sage_const_0 ][k]*B,_sage_const_1 ))
...
def AB_grass_restrict_ok(W,adim,rcandidate,fast=False):
    ...
    eqs = matrix_rank_le_eqs(M, M.dimensions()[_sage_const_0 ] - rcandidate,_sage_const_0  if fast else _sage_const_1 )
    if _sage_const_1  in eqs: return None
    M = matrix_11_to_21(transpose_tensor(W,adim),bdim)
    eqs.extend(matrix_rank_le_eqs(M, M.dimensions()[_sage_const_0 ] - rcandidate,_sage_const_0  if fast else _sage_const_1 ))
...
def M_to_number_field(M):
    ...
    M = {k[_sage_const_0 ]:matrix(S,k[_sage_const_1 ][_sage_const_0 ],k[_sage_const_1 ][_sage_const_1 ],v) for k,v in MM.items()}
    return M,S
...
def print_sparse_by_columns(m,st=None,header=None,sort=True):
    ...
    return '\n'.join(header(j) + ' + '.join(st(i,j,m[i,j]) 
        ...
        for j in range(m.dimensions()[_sage_const_1 ]))
...
def psbc4(n):
    return lambda i,j,e: '%s a%d%db%d%d' %\
            (str(e), i//n**_sage_const_3 , (i//n**_sage_const_2 )%n, (i//n)%n, i%n)

def psbc6(n):
    return lambda i,j,e: '%s a%d%db%d%dc%d%d' %\
            (str(e), i//n**_sage_const_5 , (i//n**_sage_const_4 )%n, (i//n**_sage_const_3 )%n, (i//n**_sage_const_2 )%n, (i//n)%n, i%n)
    ...
    for c in candidates:
        ...
        try:
            ...
            try:
                ...
                if R.ngens() == _sage_const_1  and not R.is_field(): 
...
def print_upset(P,up,verbose=_sage_const_0 ):
    up = dict(up)
    tot={p:i for i,p in enumerate(reversed(P.linear_extension()))}
    def lab(p):
        if verbose==_sage_const_2 :
            return '%s,%s:%d/%d' % (str(p[_sage_const_0 ]),str(p[_sage_const_1 ]),up.get(p,_sage_const_0 ),p[-_sage_const_1 ])
        elif verbose==_sage_const_1 :
            return '%s:%d/%d' % (str(p[_sage_const_0 ]),up.get(p,_sage_const_0 ),p[-_sage_const_1 ])
        else:
            return '%d:%d/%d' % (tot[p],up.get(p,_sage_const_0 ),p[-_sage_const_1 ])
...
def closed_under_raising(reps, ghwv):
    r = matrix_rank(block_matrix([[ghwv]+[x*ghwv for x in reps[_sage_const_0 ]+reps[_sage_const_2 ]]]))
    return r == ghwv.dimensions()[_sage_const_1 ]
